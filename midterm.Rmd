...
r2_B <- R2(enet_pred_B, all_datB$log_PFS)
...
performance_table <- data.frame(
  ...
  r2 = c(round(r2_A, 4), round(r2_B, 4))
)

colnames(performance_table) <- c("Dataset", "RMSE", "R^2")
...
```{r}
## Compare datA & datB model's exponentiated values

# extract coefficients from both models
coefs_A <- as.data.frame(as.matrix(coef(enet.fit$finalModel, 
                                         enet.fit$bestTune$lambda)))
coefs_B <- as.data.frame(as.matrix(coef(enet.test.b$finalModel, 
                                         enet.test.b$bestTune$lambda)))
...
# exponentiate both and compute difference on exponentiated scale
coef_comparison$Exp_datA   <- round(exp(coef_comparison$Coef_datA), 4)
coef_comparison$Exp_datB   <- round(exp(coef_comparison$Coef_datB), 4)
coef_comparison$Difference <- round(coef_comparison$Exp_datB - coef_comparison$Exp_datA, 4)

# sort by absolute difference
coef_comparison <- coef_comparison[order(abs(coef_comparison$Difference), 
                                          decreasing = TRUE), ]
rownames(coef_comparison) <- NULL

# keep only exponentiated columns
coef_comparison <- coef_comparison[, c("Predictor", "Exp_datA", "Exp_datB", "Difference")]

# display table
coef_comparison |>
  kable(caption = "Comparison of Elastic Net Coefficients: datA vs datB",
        col.names = c("Predictor",
                      "datA (Exp. Coefficient)",
                      "datB (Exp. Coefficient)",
                      "Difference")) |>
  kable_styling(full_width = FALSE) |>
  add_footnote("Exponentiated coefficients represent multiplicative effects on PFS. 
                Values above 1 indicate longer PFS, below 1 indicate shorter PFS. 
                Difference = datB minus datA on the exponentiated scale.
                Values of 1.000 indicate the variable was excluded by elastic net regularization.",
               notation = "none")
```
